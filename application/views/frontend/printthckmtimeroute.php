...
/*$sqlloc = "SELECT LRNO, THCNO, DATE_FORMAT(DRSdate, '%d-%b-%Y') DRSdate, time(DRSDT) as DRSDT, VehicleNo, DriverName, DriverMobile, VendorName, BalanceFreight, Hamali, FreightCharge, Advance,DRSKM,
...
FROM vtcpod   t1 JOIN CityMaster t2 ON t1.`Place`=t2.CityNameEng  where THCNO ='$THCNO' ";
...
<!--<div>
 ...
 <div id="directions-panel"></div>
   ...
   if (isset($_POST['Submit'])) {
    ...
    $sql = "SELECT MAX(CAST(SUBSTRING(THCNO, 13, 6) AS UNSIGNED)) FROM vtcpod where THCNO LIKE '%$userdepo/$fyear%'";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_row($result);
    ...
    $THCNO = "DS/$userdepo/$fyear/" . str_pad($id, 6, 0, STR_PAD_LEFT);

    $sql = $updatesql;
    foreach ($_POST['LRNO'] as $lr) {
        $sql .= "INSERT INTO vtcpod(LRNO,THCNO,DRSdate,DRSDT, Cosigner,Place,Consignee,VehicleNo,DriverName,DriverMobile,VendorName,FreightCharge,Advance,BalanceFreight,Hamali,HamaliType,FreightType, StartingKM, DRSKM, LicenseNo, LicenseExp,Location,CreatedBy,CEWBNo) 
        VALUES ('$lr','$THCNO',STR_TO_DATE('$DRSdate', '%d / %m / %Y'),CONVERT_TZ(Now(),'+00:00','+05:30'),getConsignor('$lr'),getPlace('$lr'),getCosignee('$lr'),'$VehicleNo','$DriverName',
        ...
        $sql .= "UPDATE LR SET Status = 3, CurrentLocation = '$userdepo', NextLocation='$userdepo', DRS_THCNO='$THCNO' WHERE LRNO = '$lr';";
    }
    $sql .="INSERT INTO THCNO(THCNO) VALUES ('$THCNO');";
    ...
    vtcpod.ToPay = IF(LR.PayBasis = 'TO PAY', LR.DocketTotal,0) WHERE THCNO = '$THCNO';";
    ...
    INNER JOIN Vehicle on vtcpod.VehicleNo = Vehicle.Vehicle_No set DriverMaster.CloseTrip=0, Vehicle.CloseTrip=0,vtcpod.CloseTrip = 0 WHERE THCNO = '$THCNO';";
    ...
    if ($flag) {
        mysqli_commit($conn);
        echo "<p style='color:green;text-align: center;font-size: 20px;'>DRS No. $THCNO created successfully.</p>";
    } else {
...
echo "<input  type='button' onclick=\"window.open('voucheradvance.php?THCNO=$THCNO','_blank','width=1200,height=600');\" value='Print Voucher' />&nbsp;&nbsp;&nbsp;&nbsp;";
$CEWBNo='';
if ($CEWBNo != "0" && $CEWBNo != "")
...
WEB: WWW.VTC3PL.COM</b></td><td colspan=2 rowspan=2><img alt='testing' src='barcode.php?codetype=Code128&size=40&text=" .$drsdata->THCNO. "&sizefactor=1'/></td></tr>
<tr><td colspan=3 rowspan=2 align='middle'><b>CIN NUMBER : U60200PN2012PTC142997<br>
GST NUMBER : 27AAECV0781E1ZD</b></td></tr>                         
<tr><td><b>DRS/THC NO</b></td><td style='font-size:1.5em;font-weight: bold;'>" .$drsdata->THCNO. "</td>
</tr>                               
...
<tr><td>$i</td><td>" . $drsdata->Place. "</td><td>" . $thcdatanew->Consignee. "</td><td>" .$thcdatanew->LRNO. "</td><td>" .$thcdatanew->InvoiceNo. "</td><td>" . $thcdatanew->Qty. "</td>
<td>" . $thcdatanew->Weight. "</td><td>" .$drsdata->ToPay . "</td><td>" . $thcdatanew->Consignor. "</td><td>" . $drsdata->BookingDate . "</td><td>$dist</td><td>".Round($time1,3)." hours</td></tr>";
